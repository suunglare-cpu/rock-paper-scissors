...
bool Position::legal(Move m) const {
  ...
  // A non-empty captureTargets entry restricts a piece to the listed victim types.
  if (capture(m))
  {
      PieceSet targets = capture_targets(type_of(moved_piece(m)));
      Square captureSquare = type_of(m) == EN_PASSANT ? capture_square(to) : to;
      if (targets && !(targets & type_of(piece_on(captureSquare))))
          return false;
  }
...
bool Position::pseudo_legal(const Move m) const {
  ...
  if (capture(m))
  {
      PieceSet targets = capture_targets(type_of(pc));
      if (targets && !(targets & type_of(piece_on(to))))
          return false;
  }
