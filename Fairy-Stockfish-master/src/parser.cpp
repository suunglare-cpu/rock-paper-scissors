...
Variant* VariantParser<DoCheck>::parse(Variant* v) {
    ...
    const auto& it_capture_targets = config.find("captureTargets");
    if (it_capture_targets != config.end())
    {
        char mover, separator;
        std::string targets;
        bool valid = true;
        std::stringstream ss(it_capture_targets->second);
        while (ss >> mover)
        {
            size_t moverIndex = v->pieceToChar.find(toupper(mover));
            if (!(ss >> separator) || separator != ':' || !(ss >> targets) || moverIndex == std::string::npos)
            {
                valid = false;
                break;
            }
            ...
            {
                size_t targetIndex = v->pieceToChar.find(toupper(target));
                if (targetIndex == std::string::npos)
                {
                    valid = false;
                    break;
                }
                targetSet |= PieceType(targetIndex);
            }
            if (!valid)
                break;
            v->captureTargets[moverIndex] = targetSet;
        }
        if (DoCheck && !valid)
            std::cerr << "captureTargets - Invalid mapping: " << it_capture_targets->second << std::endl;
    }
